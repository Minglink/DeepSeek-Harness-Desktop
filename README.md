...
- **一键安装**：双击安装包即用，内置 Node 24 运行时与全量离线依赖树（33,800+ 文件），零软链，无需任何开发环境。
- **全新 0.1.2 架构适配**：全面适配官方 0.1.2 Launch Token 安全认证握手机制，秒级加载防黑屏。
- **插件市场「一键安装联动协议」支持 (`dsh://`)**：
  - 支持从网页端、文档、第三方服务一键安全唤起客户端并安装插件。
  - 具备原生弹窗展示插件信息与权限声明，用户点击“开始安装”后自动下载、解压并配置 Cordis Loader，热重启立即生效。
- **数据平滑过渡**：100% 完整保留用户历史会话数据（`~/.dsh/sessions`）与 API 凭据，自动清理旧版不兼容残留插件。
